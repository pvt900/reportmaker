...
class ManualReporter:
    ...
    def worktracker_scanner(self):
        ...
        sheet = wb.active
        month = int(input("Please Enter the Month you want to scan ( Enter it Numberically): "))
        startd = int(input("Please enter the starting date you want scanned: "))
        endd = int(input("Please enter the ending date you want scanned: "))
        if month < 10:
            month2 = "0"+str(month)
        else:
            month2 = str(month)
        for date in range(startd,endd+1):
            if date < 10:
                fixedD = "0"+str(date)
            else:
                fixedD = str(date)
            for row in sheet.iter_rows():#This loop iterates through the Worktracker file. To change the month modify the value below.
                #print("2019-"+(month2)+"-"+(fixedD))
                if ("2019-"+(month2)+"-"+(fixedD)) in  str(row[1].value):  #its in the Format of "Year-Month" so, to Scan for Feburary 2020 you'd put "2020-02"
                    if row[7].value != "Std Type":
                        if row[7].value is "T":
                            ...
                            self.me_approval += 1
                    if float(row[12].value) < 0:
                        self.costhr += float(row[12].value)
def main():
