    ...
    def check_rows(self):
       ...
       It also creates a second list of changed items that suffered a change in the Std/Val categories.
        ...
        for saprow in saprows:# For Each Row in the Old Report
            ...
            wtrow = indexed_wt.get(key, saprow)#For Each Wtrow with Old Report Rows thathave Matching Keys
    ...
    def worktracker_scanner(self):
        ...
        for row in sheet.iter_rows():#This loop iterates through the Worktracker file. To change the month modify the value below.
            if "Date of Ch" not in str(row[1].value):
                ...
                date = row[1].value
                if (lower <= date <= upper):  #its in the Format of "Year-Month" so, to Scan for Feburary 2020 you'd put "2020-02"
